...
def best_model_name(args, for_load=False):
    ...
    cond_embed_name = args.sd + \
        '/%s_%s%s.cond_embed' % (new_data, mode, use_emb)
...
def to_batch_seq(sql_data, table_data, idxes, st, ed, db_content=0, ret_vis_data=False):
    ...
    for i in range(st, ed):
        ...
        ans_seq.append((sql['sql']['agg'],
                        sql['sql']['sel'],
                        # number of conditions + selection
                        len(sql['sql']['conds']),
                        # col num rep in condition
                        tuple(x[0] for x in sql['sql']['conds']),
                        tuple(
                            x[1] for x in sql['sql']['conds'])))  # op num rep in condition, then where is str in cond?
        query_seq.append(sql['query_tok'])  # real query string toks
        # list of conds (a list of col, op, str)
...
def epoch_exec_acc(model, batch_size, sql_data, table_data, db_path, db_content):
    ...
    while st < len(sql_data):
        ...
        q_seq, col_seq, col_num, ans_seq, query_seq, gt_cond_seq, q_type, col_type, raw_data = \
            to_batch_seq(sql_data, table_data, perm, st,
                         ed, db_content, ret_vis_data=True)
    ...
    while st < len(sql_data):
        ...
        q_seq, col_seq, col_num, ans_seq, query_seq, gt_cond_seq, q_type, col_type, \
            raw_data = to_batch_seq(
                sql_data, table_data, perm, st, ed, db_content, ret_vis_data=True)
        raw_q_seq = [x[0] for x in raw_data]
        raw_col_seq = [x[1] for x in raw_data]
        query_gt, table_ids = to_batch_query(sql_data, perm, st, ed)
        gt_sel_seq = [x[1] for x in ans_seq]
        score = model.forward(q_seq, col_seq, col_num,
                              q_type, col_type, pred_entry)
        pred_queries = model.gen_query(score, q_seq, col_seq,
...
def load_comb_wemb(fn1, fn2):
    ...
    comb_emb = {k: wemb1.get(k, 0) + wemb2.get(k, 0)
                for k in set(wemb1) | set(wemb2)}
...
def load_concat_wemb(fn1, fn2):
    ...
    comb_emb = {k: np.concatenate((wemb1.get(k, backup), wemb2.get(k, backup)), axis=0) for k in
                set(wemb1) | set(wemb2)}
                    ...
                    if cts not in word_to_idx:
                        ...
                        for w in col:
                            if w in word_emb:
                                ...
                                emb_list.append(
                                    np.zeros(N_word, dtype=np.float32))
