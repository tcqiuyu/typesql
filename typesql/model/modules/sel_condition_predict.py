...
class SelCondPredictor(nn.Module):
    ...
    def forward(self, x_emb_var, x_len, col_inp_var, col_len, x_type_emb_var, gt_sel):
        ...
        sel_score = self.sel_out(self.sel_out_K(K_sel_expand) + \
                                 self.sel_out_col(e_col)).squeeze(2)
        ...
        if gt_sel is None:
            tmp = sel_score.data.cpu().numpy()
            gt_sel = np.argmax(tmp, axis=1)
        # gt_sel (B)
        ...
        att_sel_val = torch.bmm(self.col_att(h_enc), chosen_e_col.unsqueeze(2))
        att_sel_val = att_sel_val.squeeze(2)
        ...
        cond_col_score = self.cond_col_out(self.cond_col_out_K(K_cond_col)
                                           ...
                                           + self.cond_col_out_sel(
            K_sel_agg.unsqueeze(1).expand_as(K_cond_col)))
        cond_col_score = cond_col_score.squeeze(2)
